...
*  there is no validation or conversion of tags.<br/>
...
public class GedcomParser implements XMLReader, Locator {
        ...
        try {
          ...
          while ( (line=reader.readLine() ) != null ) {
            ...
            if (line.length() > 0) {
                ...
                level = GedcomParser.firstWord(line);
                do {
                  try {
                ...
                } while (thislevel < 0 && level.length() > 0);
                if (thislevel < 0) {
                ...
                line = GedcomParser.remainder(line);
                token1 = GedcomParser.firstWord(line);
                line = GedcomParser.remainder(line);
                ...
                if (token1.startsWith("@")) {
                    ...
                    tag = GedcomParser.firstWord(line);
                    line = GedcomParser.remainder(line);
                } else {
                    iden = "";
                    tag = token1;
                ...
                xref = "";
                if ( line.startsWith("@")) {
                    token2 = GedcomParser.firstWord(line);
  ...
  /**
    ...
    */
    private static String firstWord(String inp)
    {
        int i;
        i = inp.indexOf(' ');
        if (i==0) return GedcomParser.firstWord(inp.trim());
        if (i<0) return inp;
  ...
  /**
    ...
    */
    ...
    {
        final int i = inp.indexOf(' ');
        if (i==0) {
            return GedcomParser.remainder(inp.trim());
        } else if (i < 0) {
            return "";
        }
